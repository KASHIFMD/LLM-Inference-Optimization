...
#### Setup the inference api using FastAPI:

worker_config.yml file has configuration details of server like
`workers` count of workers
`port` on which port fastapi should up and running
`SERVER_IP` IP address of server on which the API will be running
...
- Start the fastapi server,
  ...
  python start.py
  ```
  It loads number of models instances whose count is same as number of workers of FastAPI
- API details:

    cURL:
    ```bash
    curl 
        --location 'http://A.B.C.D:5008/v1/infer' \
        --header 'Content-Type: application/json' \
        --data '{
            "image_url":"https://images.jdmagicbox.com/v2/comp/hyderabad/d3/040pxx40.xx40.220120145025.j7d3/catalogue/pista-house-bakery-and-restaurant-uppal-hyderabad-restaurants-oq0tzidbzu.jpg",
            "prompt": "give description",
            "is_json": 0
        }'
    ```
...
# System Design for inferencing from the Triton server:

![](./systemDesign.png)
![](./systemDesign.drawio)

# Where can Triton Inference Server be used?
...
| Level | Technique            | GitHub Showcase Name Example                                                                                                  |
| ----- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1     | KV Cache             | [`llm_kv_cache_inference_demo`](https://github.com/KASHIFMD/LLM-Inference-Optimization/tree/main/llm_kv_cache_inference_demo) |
| 1     | Float16 Inference    | `llm_fp16_speedup_comparison`                                                                                                 |
| 2     | Quantization         | `int8_quant_vs_fp16_accuracy_latency`                                                                                         |
| 2     | Layer Profiling      | `transformer_layerwise_latency_visualizer`                                                                                    |
| 3     | Dynamic Batching     | `llm_batch_scheduler_flask_server`                                                                                            |
| 4     | Speculative Decoding | `speculative_llm_generation_experiment`                                                                                       |
